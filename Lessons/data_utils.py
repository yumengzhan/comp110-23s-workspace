...
def column_vals(table: list[dict[str,str]],header:str) ->list[str]:
    """Returns values in a table colomn under a specififc header."""
    result: list[str] = []
    #step through table
    for row in table:
...
def columar(table: list[dict[str,str]]) -> dict[str,list[str]] :
    """Reformats data so that it is a dictionary with column headers as key"""
